 ...
 * section.
 */
static void __time_critical_func(temp_isr)(void)
{
 ...
 */
static void rssi_work(async_context_t* ctx, async_at_time_worker_t* wrk)
{
    int32_t val;
    ...
    PICOW_HTTP_ASSERT(async_context_add_at_time_worker_in_ms(ctx, wrk, RSSI_INTVL_MS));
}
...
void initiate_rssi(void* params)
{
    ...
    async_context_add_at_time_worker_in_ms(cyw43_arch_async_context(), &rssi_worker, 0);
...
void initiate_http(void* params)
{
    ...
    do {
        ...
        do {
             ...
             */
            cyw43_arch_poll();
            if ((link_status = cyw43_tcpip_link_status(&cyw43_state, CYW43_ITF_STA))
                != CYW43_LINK_UP)
            {
                if (link_status < 0) {
                    HTTP_LOG_ERROR("WiFi connect error status: %d", link_status);
                    break;
                }
                sleep_ms(100);
            }
    ...
    strncpy(netinfo.ip, ipaddr_ntoa(netif_ip_addr4(netif)), IPADDR_STRLEN_MAX);
    if (cyw43_wifi_get_mac(&cyw43_state, CYW43_ITF_STA, mac) == 0)
        snprintf(netinfo.mac,
                 MAC_ADDR_LEN,
     ...
     *
     * See:
     * https://slimhazard.gitlab.io/picow_http/group__server.html#ga3380001925d9eb091370db86d85f7349
     */
     ...
     */
    err =
        register_hndlr_methods(&cfg, "/netinfo", netinfo_handler, HTTP_METHODS_GET_HEAD, &netinfo);
    PICOW_HTTP_ASSERT(err == ERR_OK);

    err = register_hndlr_methods(&cfg, "/temp", temp_handler, HTTP_METHODS_GET_HEAD, NULL);
    PICOW_HTTP_ASSERT(err == ERR_OK);

    err = register_hndlr_methods(&cfg, "/picture", picture_handler, HTTP_METHODS_GET_HEAD, NULL);
    PICOW_HTTP_ASSERT(err == ERR_OK);

    err = register_hndlr_methods(&cfg, "/led", led_handler, HTTP_METHODS_GET_HEAD, NULL);
    PICOW_HTTP_ASSERT(err == ERR_OK);

    err = register_hndlr_methods(&cfg, "/rssi", rssi_handler, HTTP_METHODS_GET_HEAD, NULL);
    PICOW_HTTP_ASSERT(err == ERR_OK);
     ...
     *
     * See:
     * https://slimhazard.gitlab.io/picow_http/group__server.html#gae2b8bdf44100f13cd2c5e18969208ff5
     */
