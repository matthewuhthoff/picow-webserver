...
int main(void)
{
     ...
     * core.
     */
    ret = xTaskCreate(initiate_temp, "temp", configMINIMAL_STACK_SIZE, NULL, TASK_PRIO, &temp_task);
     ...
     */
    ret = xTaskCreate(
        initiate_rssi, "rssi", configMINIMAL_STACK_SIZE, &linkup, TASK_PRIO, &rssi_task);
    PICOW_HTTP_ASSERT(ret == pdPASS);

    ret =
        xTaskCreate(initiate_picture, "picture", configMINIMAL_STACK_SIZE, NULL, 0, &picture_task);
    PICOW_HTTP_ASSERT(ret == pdPASS);
     ...
     */
    ret = xTaskCreate(initiate_http, "http", HTTP_STACK_SIZE, NULL, TASK_PRIO, &http_task);
    PICOW_HTTP_ASSERT(ret == pdPASS);
